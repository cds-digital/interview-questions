...
namespace CDSPracticalTests
{
    public class Tests
    {
        ...
        {
            Assert.NotEqual(new List<string> { "one", "two", "three" }, instance.Shuffle(new List<string> { "one", "two", "three" }));
        }

        [Fact]
        public void CanSort()
        {
            Assert.Equal(new[] { 1, 2, 3, 4 }, instance.Sort(new[] { 1, 2, 3, 4 }));
            Assert.Equal(new[] { 1, 2, 3, 4 }, instance.Sort(new[] { 4, 3, 2, 1 }));
        }
