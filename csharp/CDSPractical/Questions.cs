...
namespace CDSPractical {
    public class Questions
    {
        ...
        {
            return source.Where(str => str.All(Char.IsDigit)).Select(str => int.Parse(str));
        }
        ...
        {
            IEnumerable<string> commonStr = first.Intersect(second);
            return commonStr.OrderByDescending(s => s.Length).FirstOrDefault();
        }
        ...
        {
            int prevNo = 0, curNo = 2;
            int total = prevNo + curNo;

            while (curNo <= 4000000)
            {
                int nxtEvenNo = 4 * curNo + prevNo;

                if (nxtEvenNo > 4000000)
                    break;
