    ...
    public class Questions
    {
        ...
        public IEnumerable<int> ExtractNumbers(IEnumerable<string> source)
        {
            ...
            foreach (var s in source)
            {
                ...
                {
                    extractedNumbers.Add(converted);
                }
            }
        ...
        {
            return first.Intersect(second).OrderByDescending(x => x.Length).First();
        }
        ...
        {
            // set the first two elements manually because they can't be derived
            List<int> values = new List<int> { 1, 2 };
            int sum = 2;

            while (values.Max() < 4000000)
            {
                int newValue = values[values.Count - 1] + values[values.Count - 2];
                values.Add(newValue);
                if (newValue % 2 == 0)
                {
                    sum += newValue;
