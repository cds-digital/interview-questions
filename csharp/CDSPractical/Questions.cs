...
namespace CDSPractical {
    public class Questions {
        ...
        {
            int y = 0;
            return source.Where(x => int.TryParse(x, out y)).Select(z => y);
            ...
            //{
            //    isConvertible =int.TryParse(item,out int x);
            //    if (isConvertible)
            //    {
            //        a.Add(x);
            //    }
        ...
        {
            //By using build in Intersect method
            return first.Intersect(second).OrderBy(x => x.Length).Last();

            //Without using Intersect method
            //return first.ToList().FindAll(x => second.ToList().Contains(x)).OrderBy(y => y.Length).Last();
        }
        ...
        public int FibonacciSum()
        {
            ...
            //Adding first 2 numbers of fibonacci series to list
            lstFibonacci.Add(1);
            lstFibonacci.Add(2);

            int index = 0;
            //creating a list with fibonacci numbers
            while (lstFibonacci[index] + lstFibonacci[index + 1] <= 4000000)
            {
                lstFibonacci.Add(lstFibonacci[index] + lstFibonacci[index + 1]);
                index++;
            }
            //sum of even valued terms in fibonacci list
            return lstFibonacci.Where(x => x % 2 == 0).Sum();
        }
        ...
        public IEnumerable<int> GenerateList()
        {
            ...
            var numThreads = 1;
            //changed numThreads value to 1. if it is 2, because of new thread same number adding twice.
